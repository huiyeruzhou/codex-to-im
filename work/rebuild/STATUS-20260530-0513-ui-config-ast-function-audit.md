...
- 最近提交：`Extract UI service routes`，包含 UI service route 模块、聚焦测试、审计产物、阶段状态和归档。
...
- 当前暂停继续扩大 UI config route 迁移，优先升级审计模型：在文件级自然聚类之外，增加函数/方法级节点、函数级调用/依赖度，并区分内聚边与外聚边。
- 已经开始但未提交的 UI config route 迁移需要保留现场；后续要么纳入同阶段验证，要么在函数级审计模型落地后重新判断是否继续。
...
- 行动条目：
  - 读取当前状态后确认上一阶段 `Extract UI service routes` 已提交，工作树仅 `AGENTS.md` 未提交。本阶段继续 `cluster-02`，审计 `ui-server.ts` 中 `mergeConfig`、`mergeChannelInstance`、`validateFeishuCredentials`、Weixin login 和 channel save/delete/test route 的依赖后，选择先迁出 `/api/config` workflow：它能把 `configToPayload`、`mergeConfig`、模型列表校验和 access token 生成移出 `ui-server.ts`，但不碰 channel save/delete/Weixin 登录的复杂绑定刷新逻辑。
  - 04:54 用户进一步纠偏：审计模型要区分内聚边/外聚边，度数改为函数粒度。因此本阶段暂停继续扩大 UI config route 迁移，先升级 `source-audit.mjs` 的函数级审计；当前未提交代码包括 `src/ui-config-routes.ts`、`src/ui/application/config.ts`、`src/__tests__/ui-config-routes.test.ts` 和 `src/ui-server.ts` 的相关修改。
  - 04:56 续跑时重新读取工作树，确认除用户侧 `AGENTS.md` 外，当前 rebuild 相关未提交现场是 `src/ui-server.ts`、`src/ui-config-routes.ts`、`src/ui/application/config.ts`、`src/__tests__/ui-config-routes.test.ts`、`work/rebuild/source-audit.mjs` 和本状态文件；下一步先确认函数级审计脚本能生成可复核产物，再决定是否把 UI config route 纳入同阶段。
  - 04:56-05:00 已把函数/方法级节点接入审计脚本，并把函数体内同文件调用和本地 import 符号使用建成函数依赖边；同聚合边标记为内聚，跨聚合边标记为外聚。首次复跑发现正则字面量会干扰 brace 匹配，导致短函数误判成超大函数，已补充 regex literal 跳过逻辑后复跑成功；当前产物显示函数节点 1630、函数依赖边 2204、内聚 1872、外聚 332。
  - 05:02 用户纠偏后，确认 regex/string scanning 不应作为最终函数级审计基础；已把 `source-audit.mjs` 改为 TypeScript compiler AST 解析 import/export、命名函数、函数表达式、箭头函数、class method、函数体本地调用和 import 符号使用。AST 复跑后的产物显示函数节点 1526、函数依赖边 1743、内聚 1485、外聚 258；差异主要来自不再把字符串/正则字面量和嵌套闭包内容误算到外层函数。
- 阶段验证和git提交（如通过）：
