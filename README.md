...
- 共享 Codex 线程：把本地可发现的 Codex thread 绑定到 IM，在 IM 中继续同一条会话。
...
查看最近 10 条本地 Codex 线程，发送：

```text
/t all
```

查看最多 200 条本地 Codex 线程。
再通过：
...
Codex Native、CLI 或 TUI 继续操作这条共享线程时，结果也会同步到 IM。
...
- `/`：查看当前聊天/当前会话诊断，包括线程、模型、模式、思考级别、共享镜像状态。
- `/status`：查看全局状态，包括通道、Bridge/UI 进程和 PID、绑定与会话数量。
- `/check`：检查当前会话健康状态；可用 `/check all` 查看运行中会话。
- `//...`：向模型发送以 `/` 开头的文本，例如 `//status` 会作为 `/status` 发给模型。
- `/t`：查看最近 10 条本地 Codex 线程。
- `/t all`：查看最多 200 条本地 Codex 线程。
- `/t n 100`：查看最近 100 条本地 Codex 线程，最多 200 条。
- `/t 1`：切换到第 1 条本地 Codex 线程。
- `/t ls`：查看当前聊天已绑定的线程，`*` 标记当前激活线程。
- `/t add <序号|thread-id|名称>`：把本地 Codex 线程加入当前聊天，不一定切换当前线程。
- `/t use <序号|thread-id|binding-id|名称>`：切换当前聊天的激活线程。
- `/t rm <序号|thread-id|binding-id|名称>`：移除当前聊天的指定绑定线程。
...
- `/his msg [N]`：把最近消息渲染成卡片发送，可临时指定本次条数。
...
- `/shell <command>`：在当前会话目录通过 `codex sandbox` 直接执行 shell 命令；默认 `workspace-write`，可用 `--sandbox read-only`，高风险命令需追加 `--force`。
- `/stop`：停止当前任务。
...
- `/new` 创建的是 IM 线程，当前只保证在 IM 中可继续；不保证会自动出现在本地 Codex 会话列表中。
- 一个会话只能绑定一个聊天，跨飞书/微信也互斥。
- 飞书附件当前支持图片和文件；视频目前按文件发送，不承诺原生预览。
- `/t` 默认只显示最近 10 条本地 Codex 线程；`/t all` 最多返回 200 条，`/t n 100` 也最多返回 200 条。

## 更多文档

- 当前架构说明：[docs/current-architecture.md](docs/current-architecture.md)
