...
describe('listCodexSessions', () => {
  it('falls back to the first real user message when session_index has no title', () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cti-codex-sessions-'));
  ...
  it('hides Codex threads whose project root is no longer in the Codex saved workspace list', () => {
    ...
    for (const [threadId, cwd, title] of [
      [visibleThreadId, 'C:\\Users\\zhangle\\WeChatProjects\\miniprogram-1', '保留的Codex 项目'],
...
describe('readCodexSessionEventStreamByFilePath', () => {
  it('builds display messages from the shared JSONL history parser', () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cti-codex-messages-'));
