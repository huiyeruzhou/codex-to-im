...
function makeEvent(
  signature: string,
  role: NonNullable<CodexMirrorRecord['role']>,
  content: string,
  timestamp = '2026-03-25T00:00:00.000Z',
  type: CodexMirrorRecord['type'] = 'message',
): CodexMirrorRecord {
  return {
    signature,
    type,
...
describe('reconcileCodexMirrorCursor', () => {
  it('initializes without replaying history', () => {
    ...
    const delta = reconcileCodexMirrorCursor(null, events);
  ...
  it('returns new events after the last seen signature', () => {
    ...
    const delta = reconcileCodexMirrorCursor({
      initialized: true,
  ...
  it('advances the cursor with appended events without rescanning history', () => {
    const nextCursor = advanceCodexMirrorCursor({
      initialized: true,
      lastEventSignature: 'b',
      lastEventTimestamp: '2026-03-25T00:00:02.000Z',
      lastEventType: 'message',
