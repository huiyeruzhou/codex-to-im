...
function findUpdatedSessionFile(
  ...
  if (params.sdkSessionId) {
    const resumed = findSessionFileByThreadId(params.sdkSessionId);
    if (resumed) {
      return {
        filePath: resumed,
        threadId: params.sdkSessionId,
...
function recordToolName(record: DesktopMirrorRecord): string {
  ...
  switch (record.type) {
    ...
    case 'plan_update':
      controller.enqueue(sseEvent('task_update', {
        ...
        sdk_session_id: context.threadId,
        tasks: record.tasks || [],
        todos: record.tasks || [],
      }));
      break;
...
async function launchTmuxCodexSession(
  ...
  console.log('[codex-tmux] Codex TUI start:', {
    ...
    resume_thread_id: params.sdkSessionId || null,
    ...
    start(controller) {
      (async () => {
        ...
        const context: TmuxRunContext = {
          ...
          threadId: params.sdkSessionId,
          sessionFilePath: params.sdkSessionId ? findSessionFileByThreadId(params.sdkSessionId) || undefined : undefined,
          nextOffset: 0,
          trailingText: '',
          nextTurnId: null,
          nextSpecialCallIds: [],
