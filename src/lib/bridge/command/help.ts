...
export function buildHelpCommandResponse(): string {
  return [
    ...
    '- `/shell <command>` 在当前会话目录用 `codex sandbox` 直接执行命令；默认允许网络；高风险命令需追加 `--force`',
    ...
    '- `/shell --sandbox read-only|workspace-write <command>` 指定本次 shell sandbox；不允许 danger-full-access；IM 自动链接会按显示文本执行',
