...
export function buildHelpCommandResponse(): string {
  return [
    ...
    '- `/t archive` 归档当前本地 Codex 会话并解除相关绑定；也可用 `/t archive 1` 或 `/t archive <bridge_session_id>` 指定会话',
    ...
    '- `/t rename <名称>` 重命名当前线程',
    '- 序号范围：`/t 1`、`/t attach 1` 和 `/t archive 1` 使用 `/t` 全局本地 Codex 会话表；`/t use 1` 和 `/t detach 1` 使用 `/t ls` 当前聊天局部绑定表',
